...
exports.verifyPayment = exports.getPaymentStats = exports.updatePaymentStatus = exports.getPaymentById = exports.getAllPayments = void 0;
...
const updatePaymentStatus = async (req, res) => {
    try {
       ...
       WHERE id = $4
       RETURNING *`, [status, razorpay_payment_id, razorpay_signature, id]);
        if (result.rows.length === 0) {
            res.status(404).json({ success: false, message: 'Payment not found' });
            return;
        }
        ...
        if (status === 'completed' && result.rows[0].appointment_id) {
            await database_1.default.query('UPDATE appointments SET status = $1 WHERE id = $2', ['confirmed', result.rows[0].appointment_id]);
        }
        res.status(200).json({
            success: true,
...
const getPaymentStats = async (req, res) => {
    try {
      ...
      FROM payments
    `);
        const monthlyRevenue = await database_1.default.query(`
      SELECT 
...
const verifyPayment = async (req, res) => {
    try {
        ...
        if (generated_signature === razorpay_signature) {
            await database_1.default.query(`UPDATE payments 
